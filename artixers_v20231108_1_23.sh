...
#   1   sección de permisos especiales,
#   0   ¿seleccionar ID al consultar usuarios?
#   *   Modificar para mejor funcionamiento en paquete Loc-OS
#   0   
...
function salir {
    clear;tput civis;seg=20
    
    for((cont=0;cont<=$seg;cont++))
    do
        xxx=$((seg-cont));tput cup 1 0;verde
        cat "$camino/complementos/agradecimientos";echo "";amarillo
        echo "¡Muchas gracias por utilizar artixers!";gris;echo ""
        
        if [[ $xxx -gt 1 ]];then
            echo "Pulsa alguna tecla para finalizar o espera "$xxx" segundos. "
        else
            echo "Pulsa alguna tecla para finalizar o espera "$xxx" segundo.  "
        fi
        
        read -t1 && echo "";tput sgr0
    done
    
    echo "";tput cnorm;exit
}
...
function permisos_especiales {
    ...
    read -sn1
    seleccion=$(dialog --stdout --title "Seleccione el directorio:" --dselect "/" 22 44)
...
function menu {
    ...
    gris;echo -n "║";blanco;tput bold;tput setab 4;echo -n "     Menú principal                                                  ";tput sgr0;blanco;echo "║"
    gris;echo -n "╠";blanco;echo "═════════════════════════════════════════════════════════════════════╣"
...
function aniadir_grupos {
    ...
    if [[ $nom_grupo == *[0-9]* ]];then
        echo "" && echo "El nombre del grupo no puede contener números."
        echo "Revise las instrucciones generales proporcionadas."
        pulsa_tecla;aniadir_grupos
    fi
...
function borrar_grupos {
    ...
    if [[ $(getent group $grupo_a_borrar) ]];then
        ...
        echo -n "Para continuar borrando grupos, pulse ";parpadear;celeste;echo -n "B";tput sgr0;blanco;echo "."
        echo "Cualquier otra tecla, vuelve al menú principal."
        read -sn1 var
